...
export const tenantApi={list:params=>api.get('/tenants',{params}),create:data=>api.post('/tenants',data),update:(id,data)=>api.patch(`/tenants/${id}`,data),setStatus:(id,status)=>api.patch(`/tenants/${id}/status`,{status}),remove:id=>api.delete(`/tenants/${id}`,{data:{confirmation:'DELETE'}}),me:()=>api.get('/tenants/me')};
export default api;
